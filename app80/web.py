...
class App80Handler(BaseHTTPRequestHandler):
            ...
            try:
                ...
                with zipfile.ZipFile(zip_buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                    for idx, fobj in enumerate(files, 1):
                        ...
                        tf_est = estimate_timeframe_minutes(candles)
                        if tf_est is None or abs(tf_est - 20) > 1.0:
                            raise ValueError(f"20m değil: {fname}")
                        shifted, _ = adjust_to_output_tz(candles, "UTC-5")
                        converted = convert_20m_to_80m(shifted)
                        ...
                        if "." in fname:
                            ...
                            out_name = base + "_80m.csv"
                        else:
                            out_name = fname + "_80m.csv"
                        out_name = out_name.strip().replace('"', "") or f"converted_{idx}_80m.csv"
                        zf.writestr(out_name, csv_bytes)

                data = zip_buf.getvalue()
                self.send_response(200)
                self.send_header("Content-Type", "application/zip")
                self.send_header("Content-Disposition", 'attachment; filename="converted_80m.zip"')
                self.end_headers()
                self.wfile.write(data)
                return
            except Exception as e:
    ...
    def _render_joker_selection(
            ...
            try:
                ...
                if total_iou > 0:
                    for offset in range(-3, 4):
                        for iou in results[offset]:
                            news_events = (
                                find_news_in_timerange(events_by_date, iou.timestamp, 80)
                                if events_by_date
                                else []
                            )
                            affecting_events = [
        ...
        self.send_header("Content-Type", "text/html; charset=utf-8")
        self.end_headers()
        self.wfile.write(page("app80 - Joker Seçimi", body, active_tab="iou"))
    ...
    def _handle_iou_final_analysis(self):
        ...
        self.send_header("Content-Type", "text/html; charset=utf-8")
        self.end_headers()
        self.wfile.write(page("app80 - Pattern Analiz", body, active_tab="iou"))
