...
def run(host: str, port: int, backend_host: str, app48_port: int, app72_port: int, app80_port: int, app120_port: int, app321_port: int, news_converter_port: int) -> None:
    backends = [
        ...
        Backend(name="news_converter", host=backend_host, port=news_converter_port, prefix="/news", description="📰 Markdown → JSON haber verisi dönüştürücü"),
    ]
    ...
    for backend in backends:
        title = backend.name
        if backend.name == "news_converter":
            title = "📰 News Converter"
        app_links[backend.name] = {
            "title": title,
