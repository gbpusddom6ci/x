...
def convert_12m_to_96m(candles: List[Candle]) -> List[Candle]:
    ...
    for candle in ordered:
        block_ts = _align_to_step(candle.ts, dtime(hour=18, minute=0), 96)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--csv", required=True, help="12m verisi içeren CSV dosya yolu")
    parser.add_argument(
        "--input-tz",
        choices=["UTC-4", "UTC-5"],
        default="UTC-5",
    ...
    if tf_est is None or abs(tf_est - 12) > 1.0:
        raise SystemExit("Girdi 12 dakikalık akış gibi görünmüyor")
