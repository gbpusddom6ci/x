...
class App96Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/iou_analyze":
            try:
                ...
                self.send_header("Content-Type", "text/html; charset=utf-8")
                self.end_headers()
                self.wfile.write(page("app96 - Hata", err_msg, active_tab="iou"))
                return
                ...
                with zipfile.ZipFile(zip_buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                    for idx, fobj in enumerate(files, 1):
                        ...
                        if "." in fname:
                            ...
                            out_name = base + "_96m.csv"
                        else:
                            out_name = fname + "_96m.csv"
                        out_name = out_name.strip().replace('"', "") or f"converted_{idx}_96m.csv"
                        zf.writestr(out_name, csv_bytes)

                data = zip_buf.getvalue()
                self.send_response(200)
                self.send_header("Content-Type", "application/zip")
                self.send_header("Content-Disposition", 'attachment; filename="converted_96m.zip"')
                self.end_headers()
                self.wfile.write(data)
                return
    ...
    def _render_joker_selection(
                ...
                if total_iou > 0:
                    for offset in range(-3, 4):
                        for iou in results[offset]:
                            news_events = (
                                find_news_in_timerange(events_by_date, iou.timestamp, 96)
                                if events_by_date
                                else []
                            )
                            affecting_events = [
        ...
        self.send_header("Content-Type", "text/html; charset=utf-8")
        self.end_headers()
        self.wfile.write(page("app96 - Joker Seçimi", body, active_tab="iou"))
    ...
    def _handle_iou_final_analysis(self):
        ...
        self.send_header("Content-Type", "text/html; charset=utf-8")
        self.end_headers()
        self.wfile.write(page("app96 - Pattern Analiz", body, active_tab="iou"))
